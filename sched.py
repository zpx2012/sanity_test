...
def tcpdump_tshark(out_dir,interface,remote_ip,remote_hostname,role,size):
    ...
    run_cmd_wtimer('tcpdump -w %s -i %s -n host %s and tcp port 80' % (os.path.join(out_dir,out_filename),interface,remote_ip),615)
    seq += 1
...
def client_sender():
    run_cmd_wtimer('%s/sanity_test/sender_client 169.235.31.181' % os.path.expanduser('~'),6000)

def server_sender(size):
    run_cmd_wtimer('%s/sanity_test/sender_server %d 0' % (os.path.expanduser('~'),size),6000)
...
#    start = datetime.datetime.strptime('2018-10-28 %s:30:00' % hour,'%Y-%m-%d %H:%M:%S') 
    ...
    intvl = 3300
    sched.add_job(tcpdump_tshark, 'interval', args=[out_dir,intf,rem_ip,rem_hn,role,500],seconds=620,start_date=start,end_date=start+datetime.timedelta(hours=1))
