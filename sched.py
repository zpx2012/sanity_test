...
def tcpdump_tshark(out_dir,role,remote_hostname,remote_ip):
    ...
    run_cmd_shell_wtimer('sudo tcpdump -w %s -i eth0 -n host %s' % (os.path.join(out_dir,out_filename),remote_ip),700)
...
if __name__ == '__main__':
    ...
    sched.add_job(tcpdump_tshark, 'interval', args=[out_dir,role,rem_hm,rem_ip],hours=1,start_date=start,end_date=end)
    if side == 'client':    
