...
def tcpdump_tshark(out_dir,interface,remote_ip,remote_hostname,role):
    ...
    run_cmd_wtimer('tcpdump -w %s -i %s -n host %s and tcp port 80' % (os.path.join(out_dir,out_filename),interface,remote_ip),10)
...
def server_sender():
    os.system('cd ~/sanity_test;sudo ./sender_server 500 0')
...
if __name__ == '__main__':
    ...
    sched.add_job(tcpdump_tshark, 'interval', args=[out_dir,intf,rem_ip,rem_hn,role],seconds=610,start_date=start,end_date=end)
    # if role == 'client':    
    #     sched.add_job(client_sender, 'date', run_date=start+datetime.timedelta(seconds=1))
