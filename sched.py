...
if __name__ == '__main__':
    ...
    seq = 0
    start = datetime.datetime.strptime('2018-10-28 %s:00:00' % hour,'%Y-%m-%d %H:%M:%S') 
    ...
    intvl = 3300
    sched.add_job(tcpdump_tshark, 'interval', args=[out_dir,intf,rem_ip,rem_hn,role],seconds=620,start_date=start,end_date=start+datetime.timedelta(hour=1))
    sched.add_job(tcpdump_tshark, 'interval', args=[out_dir,intf,rem_ip,rem_hn,role],seconds=620,start_date=start+datetime.timedelta(seconds=intvl),end_date=start+datetime.timedelta(hour=2))
    if role == 'client':    
