...
if [ "$(hostname)" == "terran" ]
then
screen -dmS ptr bash -c 'tfile=~/sanity_test_results/ptraceroute_sz1-aliyun_terran_$(date -u +%m%d%H%M)_server.txt;while true;do date -u +"%Y-%m-%d %H:%M:%S %Z" >> $tfile;sudo paris-traceroute -Q -d 33456 -p tcp 39.108.98.242 >> $tfile;done;exec bash'
