...
tfile=~/sanity_test/rs/ptraceroute_${hm}_$(hostname)_$(date -u +%m%d%H%M%S)utc_server.txt
...
echo "sudo tcpdump -w tcpdump_${hm}_$(hostname)_"'%m%d%H%M%S%z'"utc_server.pcap -G 1 -s 96 -i ens3 -n host $ip and tcp port 80;exec bash" > tmp$((++i)).sh
screen -dmS td_$hm bash tmp$i.sh
done
elif [[ $1 == c ]]; then
n=33456
i=0
screen -dmS nethog bash -c 'sudo nethogs -t eth0 &> ~/sanity_test/rs/nethogs_$(hostname)_$(date -u +"%m%d%H%M%Sutc").txt'
cd ~/packet_trace/loss_$stime
...
# tfile=~/sanity_test/rs/ptraceroute_$(hostname)_${hm}_$(date -u +%m%d%H%M%S)utc_client.txt
