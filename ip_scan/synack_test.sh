...
cat $1 | while IFS=' ' read ip port; do
    sudo traceroute -A -p $port -P tcp -f 4 -m 25 $ip >> ~/sanity_test_results/tr_terran_${ip}_${port}.txt 2>&1
    ...
    if [ ! -z "$rt" -a "$rt" != " " ]; then
        ...
        screen -dmS cl_$ip bash -c "while true;do ~/sanity_test/ip_scan/synack_client $ip $port;done"
