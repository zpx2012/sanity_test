...
//    memcpy(&ip->daddr, &destaddr4->sin_addr, sizeof(unsigned int));
    ...
    pTCPPacket.srcAddr = src->sin_addr.s_addr; //32 bit format of source address
    pTCPPacket.dstAddr = dst->sin_addr.s_addr; //32 bit format of source address
...
{    
    ...
    unsigned int msg_len, mode, interval;
    ...
    while(1){
        ...
        if(client->sin_addr.s_addr == ipHeader->saddr && client->sin_port == tcpHeader->source && client->sin_port != htons(80)
        && bytes > 0 && tcpHeader->ack == 1) {
