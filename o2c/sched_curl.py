    ...
    for i in range(0,len(lines)):
        ...
        if role == 'c':
            ...
            sched.add_job(nc_listen, 'interval', args=[40,fields[2]], seconds=intvl,
                  start_date=cur_st+datetime.timedelta(seconds=session+5), end_date=cur_st+datetime.timedelta(days=day))            
        else:
            ...
            sched.add_job(gfw_hop,'interval', args=[fields[0],fields[1],cur_st.strftime('%Y%m%d%H%M'),fields[2],fields[3]], seconds=intvl,
                   start_date=cur_st+datetime.timedelta(seconds=session+10), end_date=cur_st+datetime.timedelta(days=day))
    sched.start()
